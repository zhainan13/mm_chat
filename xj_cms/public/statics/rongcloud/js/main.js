                            ...
                            $.ajax({
                                ...
                                success: function(data){
                                    ...
                                    var userInfo={
                                        ...
                                        'avatar': '/data/upload/avatar/'+data['avatar'],
                    ...
                    $.post(rongUserInfoUrl, {'uids': uids}, function(data) {
                        ...
                        $.each(data, function(index, val) {
                            var userInfo={
                                ...
                                'avatar':'/data/upload/avatar/'+val['avatar'],
...
function rongCreateFriendInfo(userInfo) {
    ...
    var str='<div class="bjy-fl-one" data-edu="" data-avatar="'+userInfo['avatar']+'" data-username="'+userInfo['user_nicename']+'" data-uid="'+userInfo['id']+'"><div class="bjy-flo-avatar"><img class="bjy-floa-img" src="'+userInfo['avatar']+'">'+approveImg+countStr+'</div><ul class="bjy-flo-info"><li class="bjy-flo-username">'+userInfo['user_nicename']+'<span class="bjy-flou-time">'+userInfo['date']+'</span></li><li class="bjy-flo-school"></li></ul></div>';
    return str;
}
...
$(function(){
    ...
    $('.bjy-friend-list').on('click', '.bjy-fl-one', function(event) {
        ...
        var uid=$(this).attr('data-uid'),
            userInfo={
                'user_nicename': $(this).attr('data-username'),
        ...
        if (!$(this).hasClass('bjy-flo-checked')) {
            ...
            // 设置聊天框title
            $('#bjy-chat-modal .bjy-tt-name').text(userInfo['user_nicename']);
            // 获取历史记录
            get_contents(uid,userInfo,100);
    ...
    $('.bjy-cbh-send').click(function(event) {
    	...
    	if(!is_authusers(uid)){
           ...
           save_content(xbUserInfo['id'],uid,str);
    	}
    ...
    $('.bjy-rong-chat').click(function(event) {
        var userInfo={
            ...
            'user_nicename':$(this).attr('targetname'),
            'count':0,
            'date':new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds(),
        };
    ...
    function save_content(fromid,uid,str){
    	
    	$.ajax({
            url: saveContentUrl,
            type: 'POST',
            dataType: 'json',
            data: {'fromid':fromid,'uid': uid,'content':str},
            async : false,
            success: function(data){
   ...
   function is_authusers(uid){
	
		var is_au =$.inArray(uid, authusers);
	...
	function get_contents(uid,userInfo,scrollTopNumber){
	
		$.ajax({
	        url: getlogsUrl,
	        type: 'POST',
	        dataType: 'json',
	        data: {'uid': uid,'num':scrollTopNumber},
	        async : false,
	        success: function(data){
	        	var str='',
	            messageContent={};
	        	$.each(data.data, function(index, val) {
	        		...
	        		messageContent['content']=val['content'];
	        		str +=rongCreateMessage(messageContent);
	        	});
	        	//console.log(str)
	        	$('#bjy-chat-modal .bjy-cb-history').prepend(str);   
	        	rongChangeScrollHeight(scrollTopNumber*10);
	        }
     })
   }
